...
app.config['MAIL_USERNAME'] = os.environ.get('NIDS_MAIL_USERNAME', '')
app.config['MAIL_PASSWORD'] = os.environ.get('NIDS_MAIL_PASSWORD', '')
app.config['MAIL_DEFAULT_SENDER'] = os.environ.get('NIDS_MAIL_USERNAME', '')
mail = Mail(app)
...
if not app.config.get('MAIL_USERNAME') or not app.config.get('MAIL_PASSWORD'):
    print("WARNING: Email OTP is not configured.")
if not TWILIO_ACCOUNT_SID or not TWILIO_AUTH_TOKEN or not TWILIO_PHONE_NUMBER:
    print("WARNING: SMS OTP is not configured.")
...
def send_reset_otp_email(email, otp):
    if not app.config.get('MAIL_USERNAME') or not app.config.get('MAIL_PASSWORD'):
        print("Email is not configured. Set NIDS_MAIL_USERNAME and NIDS_MAIL_PASSWORD.")
        return False
    try:
        msg = Message(
            ...
            recipients=[email],
            body=(
                f"Your AI NIDS password reset OTP is:\n\n{otp}\n\n"
        ...
        print("Email OTP send failed:", type(exc).__name__, repr(exc))
        return False
...
def _phones_match(phone_a, phone_b):
    a = _normalize_phone_number(phone_a)
    b = _normalize_phone_number(phone_b)
    return bool(a and b and a == b)


def send_reset_otp_sms(phone, otp):
    if not TWILIO_ACCOUNT_SID or not TWILIO_AUTH_TOKEN or not TWILIO_PHONE_NUMBER:
        print("Twilio is not configured. Set TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN and TWILIO_PHONE_NUMBER.")
        return False, None
    if Client is None:
        print("Twilio package is not installed. Run: pip install twilio")
        return False, None
    ...
    try:
        ...
        print("Twilio SMS send failed:", type(exc).__name__, repr(exc))
        print("For a Twilio trial account, the destination number must be verified.")
        return False, None
...
def forgot_password():
    if request.method == "POST":
        recovery_type = request.form.get("recovery_type", "email").strip().lower()
        recovery_value = request.form.get("recovery_value", "").strip()

        if recovery_type not in {"email", "phone"}:
            recovery_type = "email"
        ...
        if recovery_type == "phone":
            ...
            user = next((row for row in rows if _phones_match(row[1], recovery_value)), None)
            target = user[1] if user else recovery_value
        else:
            normalized_email = recovery_value.lower()
            cursor.execute(
                "SELECT id, email FROM users WHERE LOWER(email)=?",
                (normalized_email,)
            )
            user = cursor.fetchone()
            target = user[1] if user else normalized_email
        ...
        if recovery_type == "phone":
            sent, _message = send_reset_otp_sms(target, otp)
            if not sent:
                return render_template(
                    "forgot_password.html",
                    error=("❌ SMS could not be sent. Check your Twilio configuration "
                           "and verify the destination number if using a Twilio trial account."),
                    recovery_type=recovery_type,
                    recovery_value=recovery_value,
                )
        else:
            sent = send_reset_otp_email(target, otp)
            if not sent:
                return render_template(
                    "forgot_password.html",
                    error=("❌ Email could not be sent. Check your Gmail address, "
                           "Gmail App Password, and SMTP configuration."),
                    recovery_type=recovery_type,
                    recovery_value=recovery_value,
                )

        session["reset_user_id"] = user[0]
        session["reset_target"] = target
...
def register_user():
    ...
    phone = _normalize_phone_number(request.form.get("phone", ""))
